/**
 * Krince Film Archive — Google Apps Script backend (V4.1 auto-save)
...
function doGet(e) {
  ...
  try {
    ...
    let data;
    switch (action) {
      case 'health': data = {ok:true, service:'Krince Film Archive V4.1'}; break;
      ...
      case 'personSearch': data = tmdbPersonSearch_(e.parameter.q || ''); break;
...
function handleJsonpWrite_(op, payload) {
  switch (op) {
    ...
    case 'addTmdbFilm': return addTmdbFilm_(payload || {});
    ...
    case 'deleteFilm': return deleteFilm_(payload.id || payload.filmId || '');
    case 'remapFilm': return remapFilm_(payload.oldId || payload.id || '', payload.newTmdbId || payload.tmdbId || '');
    case 'ignoreMigration': return ignoreMigration_(payload.legacyId || '');
...
function doPost(e) {
  ...
  try {
    ...
    switch (body.action) {
      ...
      case 'addTmdbFilm': data = addTmdbFilm_(body); break;
      ...
      case 'deleteFilm': data = deleteFilm_(body.id || body.filmId || ''); break;
      case 'remapFilm': data = remapFilm_(body.oldId || body.id || '', body.newTmdbId || body.tmdbId || ''); break;
      case 'ignoreMigration': data = ignoreMigration_(body.legacyId || ''); break;
...
function getLibrary_() {
  ...
  const migratedLegacyIds = migrationRows.filter(r => ['matched','deleted','ignored'].includes(String(r.match_status || '').toLowerCase())).map(r => String(r.legacy_id));
  return {migratedLegacyIds:migratedLegacyIds, films:films.map(f => {
    ...
    return {
      ...
      tmdbVoteAverage:f.tmdb_vote_average === '' || f.tmdb_vote_average == null ? null : Number(f.tmdb_vote_average),
      tmdbVoteCount:f.tmdb_vote_count === '' || f.tmdb_vote_count == null ? null : Number(f.tmdb_vote_count),
...
function tmdbSearch_(query) {
  ...
  return {results:(zh.results || []).slice(0,16).map(r => ({
    ...
    tmdbVoteAverage:r.vote_average == null ? null : Number(r.vote_average), tmdbVoteCount:r.vote_count == null ? null : Number(r.vote_count)
  }))};
}

function tmdbPersonSearch_(query) {
  query = String(query || '').trim();
  if (!query) return {results:[]};
  const r = tmdb_('/search/person', {query:query, language:'zh-HK', include_adult:'false', page:1});
  return {results:(r.results || []).slice(0,20).map(p => ({
    id:p.id, name:p.name || p.original_name || '',
    knownForDepartment:p.known_for_department || '',
    popularity:p.popularity || 0, profilePath:p.profile_path || ''
  }))};
}
...
function tmdbDetails_(id) {
  ...
  return {
    ...
    overviewZh:zh.overview || '', overviewEn:en.overview || '',
    tmdbVoteAverage:zh.vote_average == null ? null : Number(zh.vote_average),
    tmdbVoteCount:zh.vote_count == null ? null : Number(zh.vote_count),
...
function tmdbDiscover_(p) {
  ...
  return {page:r.page, totalPages:r.total_pages, results:(r.results || []).map(x => ({
    ...
    tmdbVoteAverage:x.vote_average == null ? null : Number(x.vote_average), tmdbVoteCount:x.vote_count == null ? null : Number(x.vote_count)
  }))};
}
...
function upsertFilmAndWatch_(film) {
  ...
  const filmRow = {
    ...
    tmdb_vote_average:enriched.tmdbVoteAverage == null ? (existingFilm.tmdb_vote_average == null ? '' : existingFilm.tmdb_vote_average) : enriched.tmdbVoteAverage,
    tmdb_vote_count:enriched.tmdbVoteCount == null ? (existingFilm.tmdb_vote_count == null ? '' : existingFilm.tmdb_vote_count) : enriched.tmdbVoteCount
  };
...
function refreshFilmMetadataOnly_(tmdbId) {
  tmdbId = String(tmdbId || '').trim();
  if (!tmdbId) throw new Error('TMDB id required');
  setupDatabase();
  const ss = getSpreadsheet_();
  const sh = ss.getSheetByName(FILMS_SHEET);
  const internalId = 'tmdb-' + tmdbId;
  const old = getByKey_(sh, FILM_HEADERS, 'internal_id', internalId);
  if (!old) return {ok:false, missing:true};
  const d = tmdbDetails_(tmdbId);
  const now = new Date().toISOString();
  const row = Object.assign({}, old, {
    title_zh_hk:d.titleZh || old.title_zh_hk || '', title_en:d.titleEn || old.title_en || '',
    original_title:d.originalTitle || old.original_title || '', release_year:d.year || old.release_year || '',
    release_date:d.releaseDate || old.release_date || '', regions:d.region || old.regions || '',
    original_language:d.originalLanguage || old.original_language || '', genres:joinPipe_(d.genres) || old.genres || '',
    director:d.director || old.director || '', cast:joinPipe_(d.cast) || old.cast || '', runtime:d.runtime || old.runtime || '',
    poster_path:d.posterPath || old.poster_path || '', backdrop_path:d.backdropPath || old.backdrop_path || '',
    overview_zh_hk:d.overviewZh || old.overview_zh_hk || '', overview_en:d.overviewEn || old.overview_en || '',
    tmdb_vote_average:d.tmdbVoteAverage == null ? old.tmdb_vote_average || '' : d.tmdbVoteAverage,
    tmdb_vote_count:d.tmdbVoteCount == null ? old.tmdb_vote_count || '' : d.tmdbVoteCount,
    updated_at:now
  });
  upsertByKey_(sh, FILM_HEADERS, 'internal_id', internalId, row);
  return {ok:true,id:internalId};
}

function addTmdbFilm_(payload) {
  const tmdbId = Number(payload.tmdbId || payload.id || 0);
  if (!tmdbId) throw new Error('TMDB id required');
  const watch = payload.watch || {};
  const d = tmdbDetails_(tmdbId);
  return upsertFilmAndWatch_(Object.assign({}, d, {
    status:watch.status || 'complete', watchedDate:watch.watchedDate || '',
    watchedFrom:watch.watchedFrom || '', watchedTo:watch.watchedTo || '',
    datePrecision:watch.datePrecision || 'unknown', platform:watch.platform || '',
    rating:watch.rating == null ? null : Number(watch.rating), note:watch.note || '',
    isRewatch:truthy_(watch.isRewatch)
  }));
}
...
function migrateLegacyBatch_(records) {
  ...
  (records || []).forEach(rec => {
    try {
      ...
      const priorStatus = String(prior.match_status || '').toLowerCase();
      if (priorStatus === 'matched') {
        // V4 backfills newer TMDB metadata (including vote average/count) without touching the user's watch record.
        if (prior.tmdb_id) {
          const filmRow = getByKey_(ss.getSheetByName(FILMS_SHEET), FILM_HEADERS, 'internal_id', 'tmdb-' + prior.tmdb_id) || {};
          if (filmRow.tmdb_vote_average === '' || filmRow.tmdb_vote_average == null) {
            try { refreshFilmMetadataOnly_(prior.tmdb_id); } catch (_) {}
          }
        }
        skipped++; return;
      }
      if (['deleted','ignored'].includes(priorStatus)) { skipped++; return; }
...
function remapFilm_(oldInternalId, newTmdbId) {
  oldInternalId = String(oldInternalId || '').trim();
  newTmdbId = String(newTmdbId || '').trim();
  if (!oldInternalId) throw new Error('Old film id required');
  if (!newTmdbId) throw new Error('New TMDB id required');
  setupDatabase();
  const ss = getSpreadsheet_();
  const filmsSheet = ss.getSheetByName(FILMS_SHEET);
  const watchesSheet = ss.getSheetByName(WATCHES_SHEET);
  const migrationSheet = ss.getSheetByName(MIGRATION_SHEET);
  const oldFilm = getByKey_(filmsSheet, FILM_HEADERS, 'internal_id', oldInternalId);
  if (!oldFilm) throw new Error('Original film not found');
  const oldTmdbId = String(oldFilm.tmdb_id || '').trim();
  if (oldTmdbId === newTmdbId) return {ok:true, changed:false, newId:oldInternalId, tmdbId:Number(newTmdbId)};

  const d = tmdbDetails_(newTmdbId);
  const oldWatches = sheetObjects_(watchesSheet).filter(w => String(w.film_id || '') === oldInternalId);
  const primary = oldWatches[0] || {};
  const result = upsertFilmAndWatch_(Object.assign({}, d, {
    status:primary.status || 'complete', watchedDate:primary.watched_date || '',
    watchedFrom:primary.watched_from || '', watchedTo:primary.watched_to || '',
    datePrecision:primary.date_precision || 'unknown', platform:primary.platform || '',
    rating:primary.rating === '' || primary.rating == null ? null : Number(primary.rating),
    note:primary.note || '', isRewatch:truthy_(primary.is_rewatch)
  }));
  const newInternalId = result.id;

  deleteRowsWhere_(watchesSheet, WATCH_HEADERS, row => String(row.film_id || '') === oldInternalId);
  deleteRowsWhere_(filmsSheet, FILM_HEADERS, row => String(row.internal_id || '') === oldInternalId);

  if (migrationSheet && oldTmdbId) {
    const rows = sheetObjects_(migrationSheet);
    rows.forEach(row => {
      if (String(row.tmdb_id || '') !== oldTmdbId) return;
      if (String(row.match_status || '').toLowerCase() !== 'matched') return;
      upsertByKey_(migrationSheet, MIGRATION_HEADERS, 'legacy_id', String(row.legacy_id), Object.assign({}, row, {
        match_status:'matched', tmdb_id:newTmdbId, matched_title:d.titleZh || '', matched_year:d.year || '',
        confidence:'manual-remap', review_reason:'', last_error:'', updated_at:new Date().toISOString()
      }));
    });
  }
  return {ok:true, changed:true, oldId:oldInternalId, newId:newInternalId, tmdbId:Number(newTmdbId)};
}

function ignoreMigration_(legacyId) {
  ...
  const row = getByKey_(sh, MIGRATION_HEADERS, 'legacy_id', legacyId) || {legacy_id:legacyId};
  row.match_status = 'ignored';
  row.review_reason = '由使用者略過';
  row.last_error = '';
  row.updated_at = new Date().toISOString();
  upsertByKey_(sh, MIGRATION_HEADERS, 'legacy_id', legacyId, row);
  return {ok:true, legacyId:legacyId};
}

function deleteFilm_(internalId) {
  internalId = String(internalId || '').trim();
  if (!internalId) throw new Error('Film id required');
  setupDatabase();
  const ss = getSpreadsheet_();
  const filmsSheet = ss.getSheetByName(FILMS_SHEET);
  const watchesSheet = ss.getSheetByName(WATCHES_SHEET);
  ...
  const film = getByKey_(filmsSheet, FILM_HEADERS, 'internal_id', internalId);
  if (!film) return {ok:true, deleted:false, id:internalId};
  const tmdbId = String(film.tmdb_id || '').trim();

  deleteRowsWhere_(watchesSheet, WATCH_HEADERS, row => String(row.film_id || '') === internalId);
  deleteRowsWhere_(filmsSheet, FILM_HEADERS, row => String(row.internal_id || '') === internalId);

  if (tmdbId && migrationSheet) {
    const rows = sheetObjects_(migrationSheet);
    rows.forEach(row => {
      if (String(row.tmdb_id || '') !== tmdbId) return;
      if (String(row.match_status || '').toLowerCase() !== 'matched') return;
      upsertByKey_(migrationSheet, MIGRATION_HEADERS, 'legacy_id', String(row.legacy_id), Object.assign({}, row, {
        match_status:'deleted',
        review_reason:'已由片庫刪除；不再自動匯入',
        updated_at:new Date().toISOString()
      }));
    });
  }
  return {ok:true, deleted:true, id:internalId, tmdbId:tmdbId || null};
}
...
function deleteRowsWhere_(sheet, headers, predicate) {
  ...
  values.forEach((vals, idx) => {
    ...
    if (predicate(obj)) rowsToDelete.push(idx + 2);
  });
...
function matchLegacyRecord_(rec) {
  ...
  const candidates = merged.map(r => ({id:r.id,title:r.title || r.original_title || '',originalTitle:r.original_title || '',year:(r.release_date || '').slice(0,4),posterPath:r.poster_path || '',backdropPath:r.backdrop_path || ''}));
...
function migrationReview_() {
  ...
  return {records:rows.map(r => ({
    ...
    matchStatus:r.match_status || '', currentTmdbId:r.tmdb_id || '', matchedTitle:r.matched_title || '', matchedYear:r.matched_year || '',
