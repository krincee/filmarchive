(() => {
  ...
  const PENDING_WRITES_KEY='krince-pending-writes-v1';
  function loadPendingWrites(){
    try{const q=JSON.parse(localStorage.getItem(PENDING_WRITES_KEY)||'[]');return Array.isArray(q)?q:[];}catch(_){return [];}
  }
  function savePendingWrites(q){localStorage.setItem(PENDING_WRITES_KEY,JSON.stringify(q||[]));state.pendingCount=(q||[]).length;}
  function pendingWriteKey(op,payload){
    const id=payload?.tmdbId||payload?.film?.tmdbId||payload?.film?.id||payload?.id||Date.now();
    return `${op}:${id}`;
  }
  function enqueuePendingWrite(op,payload){
    const q=loadPendingWrites(), key=pendingWriteKey(op,payload);
    const item={key,op,payload,createdAt:new Date().toISOString(),attempts:0};
    const i=q.findIndex(x=>x.key===key); if(i>=0)q[i]=item; else q.push(item);
    savePendingWrites(q); return item;
  }
  function removePendingWrite(key){const q=loadPendingWrites().filter(x=>x.key!==key);savePendingWrites(q);}
  function pendingTmdbIds(){return new Set(loadPendingWrites().map(x=>Number(x.payload?.tmdbId||x.payload?.film?.tmdbId||0)).filter(Boolean));}
  function isPendingLocalFilm(f){
    return loadPendingWrites().some(x=>{const tid=Number(x.payload?.tmdbId||x.payload?.film?.tmdbId||0);const fid=x.payload?.film?.id||x.payload?.id||'';return (tid&&Number(f.tmdbId)===tid)||(fid&&String(f.id)===String(fid));});
  }
  function minimalWatch(f={}){
    return {status:f.status||'complete',watchedDate:f.watchedDate||'',watchedFrom:f.watchedFrom||'',watchedTo:f.watchedTo||'',datePrecision:f.datePrecision||'unknown',platform:f.platform||'',rating:f.rating==null?null:Number(f.rating),note:f.note||'',isRewatch:!!f.isRewatch};
  }
  ...
  function renderHome(){
    ...
    const pending=state.pendingCount||loadPendingWrites().length;
    const syncText=!API?'連接後，新加入電影會即時自動儲存到 Google Sheet。':state.migrationRunning?progress:pending?`${pending} 部新加入電影等待上傳；保持連線時會自動續傳。`:legacyDone?'片庫已連接；新增電影會自動儲存，毋須再同步全個資料庫。':(needs?`舊片單仍有 ${needs} 部等待 TMDB 補資料。`:'片庫已連接；新增電影會自動儲存。');
    const syncActions=!API?`<button class="sync-primary-btn" data-connect-backend>連接資料庫 →</button>`:(legacyDone?`<button class="sync-token-reset" data-flush-pending>${pending?'立即續傳新加入電影':'資料已同步'}</button><button class="sync-token-reset" data-run-migration>舊片單資料維護</button><button class="sync-token-reset" data-reset-write-token>重新輸入同步碼</button>`:`<button class="sync-primary-btn" data-run-migration ${state.migrationRunning?'disabled':''}>${state.migrationRunning?'同步中…':'繼續搬遷舊片單 →'}</button><button class="sync-token-reset" data-reset-write-token>重新輸入同步碼</button>`);
    const syncCard=`<div class="sync-card sync-card-v23"><div class="sync-copy"><div class="sync-kicker">DATABASE · ${API?'READY':'NOT CONNECTED'}</div><strong>${API?'Google Sheet 已連接':'尚未連接 Google Sheet'}</strong><span>${syncText}</span></div>${syncActions}</div>`;
  ...
  function bind(){
    ...
    $('[data-flush-pending]')?.addEventListener('click',()=>flushPendingWrites({notify:true,refresh:true}));
  ...
  async function refreshReviewCount(){
    ...
    try{const m=await apiGet('migrationStatus');state.migrationStatus=m;state.reviewCount=Number(m.review||0)+Number(m.error||0);}catch(e){console.warn('Review status unavailable',e);}
  }
  ...
  async function openReviewRecord(rec){
    ...
    const basic=(rec.candidates||[]).slice(0,6);
    let details=[];
    if(basic.length){
      const settled=await Promise.allSettled(basic.map(async c=>{
        const cid=Number(c.id||c.tmdbId||0);
        if(cid){try{return mapApiFilm(await apiGet('tmdbDetails',{id:cid}));}catch(_){}}
        return mapApiFilm({tmdbId:cid||null,titleZh:c.titleZh||c.title||c.originalTitle||'未命名',titleEn:c.titleEn||'',originalTitle:c.originalTitle||'',year:c.year||'',posterPath:c.posterPath||''});
      }));
      details=settled.filter(x=>x.status==='fulfilled').map(x=>x.value).filter(x=>x.tmdbId);
    }
    renderReviewCandidates(rec,details,'#review-candidates');
    if(!details.length){
      const q=(rec.matchQuery||rec.rawTitle||'').trim();
      if(q.length>=2){try{const d=await apiGet('tmdbSearch',{q});renderReviewCandidates(rec,(d.results||[]).slice(0,8).map(mapApiFilm),'#review-candidates');}catch(_){}}
    }
  ...
  function renderReviewCandidates(rec,films,target){
    const box=$(target);if(!box)return;box.innerHTML=films.length?films.map(f=>`<div class="review-candidate"><div class="review-candidate-poster" style="background:${posterBg(f)}"><div class="mini-placeholder">${esc(f.titleZh)}</div>${posterImg(f,'w185')?`<img src="${posterImg(f,'w185')}" onerror="this.remove()" alt="">`:''}</div><div><strong>${esc(f.titleZh)}</strong><span>${esc(f.titleEn||f.originalTitle||'')} ${f.year?`· ${f.year}`:''}</span>${f.director?`<small>導演 · ${esc(f.director)}</small>`:''}${f.cast?.length?`<small>${f.cast.slice(0,3).map(esc).join(' · ')}</small>`:''}</div><button data-resolve-review="${esc(f.tmdbId)}">就是這套</button></div>`).join(''):`<div class="empty">暫時搵唔到候選電影，可以用下面搜尋。</div>`;
    $$('[data-resolve-review]',box).forEach(btn=>btn.onclick=()=>resolveReviewRecord(rec,Number(btn.dataset.resolveReview),btn));
  }
  async function resolveReviewRecord(rec,tmdbId,btn){
    if(!tmdbId)return toast('呢個候選缺少 TMDB ID');
    const queue=await getLegacyQueue();const meta=queue.find(x=>String(x.legacy_id)===String(rec.legacyId))||{};
    if(btn){btn.disabled=true;btn.textContent='確認中…';}
    try{
      await apiPost('resolveMigration',{record:{legacyId:rec.legacyId,tmdbId,status:meta.status||'complete',platform:meta.platform||''}});
      await refreshLiveLibraryDuringMigration();
      const data=await apiGet('migrationReview');state.reviewRecords=data.records||[];state.reviewCount=state.reviewRecords.length;
      closeModal();state.page='review';render();toast('配對已確認');
    }catch(e){if(btn){btn.disabled=false;btn.textContent='就是這套';}toast('確認失敗：'+e.message);}
  }
  ...
  async function flushPendingWrites({notify=false,refresh=false}={}){
    if(!API||state.pendingFlushing)return;
    let q=loadPendingWrites();state.pendingCount=q.length;if(!q.length){if(notify)toast('所有新加入電影已同步');render();return;}
    if(!localStorage.getItem('krince-write-token')){if(notify)openSyncTokenModal('pending');return;}
    state.pendingFlushing=true;render();let success=0;
    try{
      while(q.length){
        const item=q[0];
        try{
          await apiPost(item.op,item.payload);
          q.shift();savePendingWrites(q);success++;render();
        }catch(e){
          item.attempts=(item.attempts||0)+1;savePendingWrites(q);
          console.warn('Pending write paused',item,e);
          if(String(e.message||'').includes('Invalid write token')){localStorage.removeItem('krince-write-token');if(notify)openSyncTokenModal('pending');}
          else if(notify)toast('自動儲存暫停；之後會自動續傳');
          break;
        }
        await sleep(120);
      }
      if(refresh&&success)await refreshLiveLibraryDuringMigration();
      if(notify&&!loadPendingWrites().length)toast(`已同步 ${success} 部新加入電影`);
    }finally{state.pendingFlushing=false;state.pendingCount=loadPendingWrites().length;render();}
  }
  ...
  function openSyncTokenModal(mode='legacy'){
  ...
  function openRecordModal(f,editing=false){
    ...
    $('#save-record').onclick=async()=>{
      const btn=$('#save-record');btn.disabled=true;btn.textContent=editing?'儲存中…':'加入中…';
      ...
      if($('#m-date-unknown').checked){data.datePrecision='range';data.watchedFrom=data.watchedFrom||'2021';data.watchedTo=data.watchedTo||'2026';delete data.watchedDate;} else {data.datePrecision='exact';data.watchedDate=$('#m-date').value;data.watchedFrom='';data.watchedTo='';}
      ...
      if(API){
        const op=data.tmdbId?'addTmdbFilm':'addFilm';
        const payload=data.tmdbId?{tmdbId:data.tmdbId,watch:minimalWatch(data)}:{film:data};
  ...
  function openBulkModal(){
    ...
    $('#bulk-unknown').onclick=async()=>{
      const btn=$('#bulk-unknown');btn.disabled=true;btn.textContent='加入中…';
